...
func (t *IndexWorkspaceTool) Description() string {
	return "Index/reindex the codebase for search - USUALLY AUTOMATIC on first search. Call manually only if rag_search_code returns 'workspace not indexed' or after major code changes (git pull, branch switch). Analyzes Go, PHP, Python, HTML files and stores vectors for semantic search.\nExample: { \"file_path\": \"/path/to/project/main.go\", \"recreate\": true }"
}
